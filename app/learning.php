<?php
$action = "academy";
$tabnames = selectQuery_tc_Academy("status='open'");
$tabtype = $tabnames[0]['id'];
$tabname = $tabnames[0]['id'];
$them = 'light';
if ($userid > 0) {
    ...
    <div class="RightSideOfCode BgWhite PosRel">
        ...
        <div id="all_tabs" class="P_TabCode">
            ...
            <?php foreach ($tabnames as $tabname) {
                ...
                <div class="TabCode">
                    <?php
                    require("codeLearn.php");
                    ?>
                </div>
            <?php } ?>
            <div id="description_body" class="P_AllTextArea">
                <div class="P_TextAreaApp">
                    <textarea name="" class="TextAreaApp BgWhite AlmostBlack"></textarea>
                </div>
                <div class="P_TextArea">
...
<?php
if (isset($_GET['id'])) {
    $get_id = explode("__", $_GET['id']);
    $get_id = $get_id[1];
    echo "<script>OpenFile('$get_id.$get_id');
        document.getElementById('description_body').innerHTML = descriptions[$get_id];
        document.getElementById('description_body').style.display = 'block';</script>";
}
?>
