...
if (isset($_GET['request'])) {
    ...
    if ($request == "like") {
    ...
    }else if ($request == "removeOrder"){
        if ($userid > 0){
            $orderId = $_GET['orderId'];
            $exist = sizeof(selectQuery_tc_order("userid='$userid' and id='$orderId'"));
            if ($exist == 1){
                deleteQuery('tc_order',"userid='$userid' and id='$orderId'");
                echo 'true';
            }else{
                echo "not exist";
            }
    ...
    }else if ($request == "addOrder"){
        if ($userid > 0){
            $product_id = $_GET['product_id'];
            insertQuery('tc_order',"userid, product_id","'$userid', '$product_id'");
        }else{
            echo 'false';
        }
        ...
        if ($userid > 0){
            ...
            $exist = sizeof(selectQuery_tc_order("userid='$userid' and id='$orderId'"));
            if ($exist == 1){
                updateQuery('tc_order',"count='$count'","userid='$userid' and id='$orderId'");
                echo 'true';
            }else{
                echo "not exist";
            }
